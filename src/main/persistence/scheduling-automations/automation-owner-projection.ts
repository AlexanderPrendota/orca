...
export function automationIdsPinnedToSshTarget(state: PersistedState, targetId: string): Set<string> {
  const hostState = folderWorkspaceHostState(state)
  return new Set(
    (state.automations ?? [])
      .filter((automation) => resolveAutomationWorkspaceSshTargetId(hostState, automation.workspaceId) === targetId)
...
export function automationWorkspaceSshPin(
  ...
  const targetId = resolveAutomationWorkspaceSshTargetId(folderWorkspaceHostState(state), workspaceId)
  return targetId === undefined ? undefined : { targetId, generation: sshTargetGenerationForConnection(state, targetId) }
}
...
export function automationProjectionContext(
  ...
  return {
    ...
    workspaceHost: (automation) => resolveAutomationWorkspaceHost(folderWorkspaceHostState(state), automation.workspaceId),
    ...(usage ? { usageSummary: (id: string) => usage.get(id) ?? null } : {})
  }
}

function projectionInputs(state: PersistedState): readonly unknown[] {
  return [state.automations, state.automationRuns, state.repos, state.sshTargets, state.folderWorkspaces, state.projectGroups]
}
...
export function listAutomationsForScope(input: {
  ...
  if (scope?.kind === 'ssh') {
    ...
    if (current === undefined) throw new AutomationOwnerConflictError(AUTOMATION_OWNER_CONFLICT_CODES.targetRemoved)
    if (current !== scope.expectedTargetGeneration) throw new AutomationOwnerConflictError(AUTOMATION_OWNER_CONFLICT_CODES.ownerChanged)
  }
  const inputs = projectionInputs(input.state)
  const complete = input.cache && inputs.every((entry, index) => entry === input.cache?.inputs[index])
    ? input.cache
    : { inputs, result: projectAutomationList(input.automations, automationProjectionContext(input.state, input.storageAuthority, true)) }
  if (!scope) return { result: complete.result, cache: complete }
  const items = complete.result.items.filter((item) => automationSelectorMatchesScope(item.selector, scope))
  const ids = new Set(items.map((item) => item.automationId))
  return { result: { automations: complete.result.automations.filter((automation) => ids.has(automation.id)), items, orphanCount: complete.result.orphanCount }, cache: complete }
}

export function automationOwnerPrecondition(state: PersistedState, storageAuthority: AutomationStorageAuthority, id: string): AutomationOwnerPrecondition | null {
  const automation = (state.automations ?? []).find((entry) => entry.id === id)
  return automation ? toAutomationOwnerPrecondition(projectAutomationSelector(automation, automationProjectionContext(state, storageAuthority, false))) : null
}

export function automationChangeSelector(state: PersistedState, storageAuthority: AutomationStorageAuthority, id: string): AutomationChangeSelector | null {
  const automation = (state.automations ?? []).find((entry) => entry.id === id)
  return automation ? toAutomationChangeSelector(projectAutomationSelector(automation, automationProjectionContext(state, storageAuthority, false))) : null
}

export function automationCapturedHostIssue(state: PersistedState, storageAuthority: AutomationStorageAuthority, automation: Automation): AutomationCapturedHostIssue | null {
  return resolveAutomationCapturedHostIssue(automation, automationProjectionContext(state, storageAuthority, false))
}

export function assertAutomationOwnerFence(input: { state: PersistedState; storageAuthority: AutomationStorageAuthority; id: string; expectedOwner?: AutomationOwnerPrecondition; operation: AutomationOwnerFenceOperation }): Automation {
  const automation = (input.state.automations ?? []).find((entry) => entry.id === input.id)
  if (!automation) throw new Error('Automation not found.')
  enforceAutomationOwnerFence({ automation, expectedOwner: input.expectedOwner, operation: input.operation, context: automationProjectionContext(input.state, input.storageAuthority, false) })
  return automation
}

export function allocateSshTargetGeneration(state: StoreOwnedPersistedState, scheduleSave: () => void): number {
  const next = nextSshTargetGeneration(resolveSshTargetGenerationHighWaterMark({
    persistedCounter: state.sshTargetGenerationCounter,
    targetGenerations: (state.sshTargets ?? []).map((target) => target.generation),
    capturedGenerations: (state.automations ?? []).map((automation) => automation.executionTargetGeneration)
  }))
